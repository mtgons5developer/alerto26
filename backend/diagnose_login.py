...
try:
    ...
    try:
        ...
        print(
            f"   staff_dashboard function exists: {hasattr(views, 'staff_dashboard')}"
        )
    ...
    try:
        ...
        if admin_users.exists():
            for admin in admin_users[:3]:
                print(
                    f"     👑 {admin.username} (staff: {admin.is_staff}, active: {admin.is_active})"
                )
        else:
        ...
        if test_user:
            ...
            print("   Authentication backend test: OK")
    except Exception as e:
        ...
        for pattern in patterns:
            if hasattr(pattern, "pattern"):
                ...
                for url_name in important_urls:
                    if url_name in pattern_str or (
                        hasattr(pattern, "name") and pattern.name == url_name
                    ):
                        found_urls.append(
                            f"{prefix}{pattern_str} -> {getattr(pattern, 'name', 'No name')}"
                        )
