...
VALID_EMERGENCY_TYPES = [
    'MEDICAL', 'FIRE', 'POLICE', 'CAR_ACCIDENT', 
    'NATURAL_DISASTER', 'UTILITY', 'OTHER'
]
...
def test_create_emergency():
    ...
    if response.status_code == 200:
        ...
        if data.get('emergency'):
            ...
            if emergency.get('user'):
                print(f"   User: {emergency['user']['username']} ({emergency['user']['id'][:8]}...)")
            return True
        else:
            print(f"❌ No emergency data returned")
    ...
    for emergency_type in VALID_EMERGENCY_TYPES:
        ...
        if response.status_code == 200:
            ...
            if result.get('errors'):
                ...
                if data.get('emergency'):
                    emergency = data['emergency']
                    print(f"    ✅ Created: {emergency['code']} ({emergency['emergencyType']})")
