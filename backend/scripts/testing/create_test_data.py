    ...
    try:
        user = User.objects.create_user(
            ...
            emergency_contacts=json.dumps([
                {"name": "Emergency Contact 1", "phone": "+639001112233", "relationship": "Family"},
                {"name": "Emergency Contact 2", "phone": "+639004445566", "relationship": "Friend"}
            ]),
            medical_info=json.dumps({
                "allergies": ["Penicillin", "Peanuts"] if random.choice([True, False]) else [],
                "conditions": ["Asthma", "Hypertension"] if random.choice([True, False]) else [],
                "medications": ["Metformin", "Lisinopril"] if random.choice([True, False]) else []
            }),
...
for i, user in enumerate(provider_users[:8]):  # Use first 8 provider users
    try:
        provider = Provider.objects.create(
            user=user,
            service_types=json.dumps(service_type_combinations[i % len(service_type_combinations)]),
            certification_level=random.choice(['Basic', 'Advanced', 'Expert', 'Certified']),
            ...
            vehicle_type=random.choice(['Ambulance', 'Fire Truck', 'Police Car', 'SUV', 'Van']),
...
emergency_types = ['medical', 'fire', 'police', 'accident', 'natural_disaster', 'hazard']
philippine_cities = [
    'Manila', 'Quezon City', 'Makati', 'Taguig', 'Pasig',
    'Mandaluyong', 'San Juan', 'Paranaque', 'Las Pinas', 'Muntinlupa'
]
...
symptoms_examples = {
    ...
    'accident': ['Vehicle collision', 'Multiple injuries', 'Road blocked', 'Trapped person'],
}
...
for i in range(1001):
    try:
        ...
        assigned_provider = random.choice(providers) if providers and random.choice([True, False]) else None
        ...
        emergency = Emergency.objects.create(
            ...
            symptoms=json.dumps(random.sample(symptoms_examples.get(emergency_type, ['Emergency reported']), 
                                            random.randint(1, 3))),
            ...
            }) if emergency_type in ['medical', 'accident'] else json.dumps({}),
            ...
            attachments=json.dumps([f"photo_{j+1}.jpg" for j in range(random.randint(0, 2))]),
            is_anonymous=reporter is None,
            created_at=datetime.now() - timedelta(hours=random.randint(0, 168)),  # Up to 7 days ago
        )
        
        # Update timestamps based on status
        if emergency.status in ['dispatched', 'arrived', 'resolved']:
            emergency.dispatched_at = emergency.created_at + timedelta(minutes=random.randint(2, 15))
        
        if emergency.status in ['arrived', 'resolved']:
            emergency.arrived_at = emergency.dispatched_at + timedelta(minutes=random.randint(5, 30))
        
        if emergency.status == 'resolved':
            emergency.resolved_at = emergency.arrived_at + timedelta(minutes=random.randint(20, 120))
...
for item in Emergency.objects.values('city').annotate(count=Count('id')).order_by('-count')[:5]:
